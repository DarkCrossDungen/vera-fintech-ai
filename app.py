...
from backend.vera_engine import VERAEngine
from backend.gemma_client import (
    explain_loan,
    explain_kyc,
    explain_fraud,
    explain_sar,
...
async def lifespan(app: FastAPI):
    ...
    model = os.getenv("GEMMA_MODEL", "google/gemma-4-E2B-it")
    print(f"VERA v3.0 starting | Gemma: {'ONLINE' if gemma_online else 'OFFLINE'} | Model: {model}")
    yield

app = FastAPI(title="VERA Fintech Platform", version="3.0.0", lifespan=lifespan)
...
async def loan_decision(req: LoanDecisionRequest):
    ...
    gemma = (
        explain_loan(req.name, req.loan_amount, req.monthly_income,
...
async def fraud_check(req: FraudCheckRequest):
    ...
    gemma = explain_fraud(req.amount, req.merchant,
                          req.location, action, risk_score) if HF_TOKEN else ""
